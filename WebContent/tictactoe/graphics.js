...
function calculateTile(coords, noHrTiles, noVrTiles) {
    calculateTile(coords, noHrTiles, noVrTiles, false);
}
...
function drawO(canvas, coords) {
    var ctx = canvas.getContext('2d');
    ctx.beginPath();
    ctx.arc((coords.xStart + coords.xEnd) / 2, (coords.yStart + coords.yEnd) / 2, coords.yEnd - (coords.yStart + coords
            .yEnd) / 2 - 20, 0, 2 * Math.PI,
        false);
    ctx.lineWidth = 5;
...
function PopupBox(canvas, text, timeout) {
    ...
    this.draw = function() {
        ...
        ctx.fillStyle = 'black';
        ctx.fillText(text, canvas.width / 2, canvas.height / 3 + canvas.height / 5.8);
    }

}

PopupBox.prototype.hide = function(game) {
	if (game != null) {
		console.log('Calling game render');
		Game.State.render();
...
PopupBox.prototype.show = function(game) {
    var popup = this;
